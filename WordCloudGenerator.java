...
/**
 * Description: The WordCloudGenerator
 *
 * <p>
 * Bugs: none
 *
...
public class WordCloudGenerator {
	...
	public static void main(String[] args) {
		...
		PriorityQueueADT<KeyWord> wordCloudQueue = new ArrayHeap<KeyWord>();

		// Check the command-line arguments and set up the input and output
		if (args.length != 4) {
			System.err.println("Four arguments required: " + 
					"inputFileName outputFileName ignoreFileName maxWords");
			System.exit(1);
		}

		// make sure the input file exists and readable
		try {
		...
		// make sure the ignore file exists and readable
		try {
			File inputF2 = new File(args[2]);
			if (!inputF2.exists() || !inputF2.canRead()) {
				System.err.println("Error: cannot access file " + args[2]);
				System.exit(1);
			}
			inIgnore = new Scanner(inputF2);
		} catch (FileNotFoundException e1) {
			System.err.println("Error: cannot access file " + args[2]);
			System.exit(1);
		}

		// check if the max word cmd line is positive
		try {
		...
		DictionaryADT<KeyWord> ignore = new BSTDictionary<KeyWord>();

		while (inIgnore.hasNext()) {
			try {
				kw = new KeyWord(inIgnore.next().toLowerCase());
				ignore.insert(kw);
		...
		while (in.hasNext()) {
			...
			for (String word : words) {
				kw = new KeyWord(word.toLowerCase());
				if (ignore.lookup(kw) == null) {
					try {
						kw = new KeyWord(word.toLowerCase());
						kw.increment();
		...
		/****
		 * Print out the information about the dictionary: - # of keys - average path
		 * length - linear average path length
		 * Put the dictionary into a priority queue
		 * Use the priority queue to create a list of KeyWords of the appropriate length
		 * Generate the html output file
		 ***/
		System.out.println("# keys: " + dictionary.size());
		System.out.println("avg path length: " + dictionary.totalPathLength() / dictionary.size());
		System.out.println("linear avg path: " + (dictionary.size() + 1) / 2);

		
		for (KeyWord kwd : dictionary) {

			wordCloudQueue.insert(kwd);
		}

		BSTDictionary<KeyWord> result = new BSTDictionary<KeyWord>();
		for (int i = 0; i < maxWords; i++) {
			if (!wordCloudQueue.isEmpty()) {

				try {
					result.insert(wordCloudQueue.removeMax());
				} catch (DuplicateException e) {
				}
			} else {
				break;
...
class KeyWord2 implements Comparable<KeyWord2> {
	 ...
	 * occurences. If the word is null or an empty string, an
